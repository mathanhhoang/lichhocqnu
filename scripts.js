        ...
        function resultSearchTable(data) {
            ...
            for (var i = 0; i < data.length; i++) {
                var row = `<tr>
                                ...
                                <td><a href="https://${data[i].loptructuyen}" target="_blank">${data[i].loptructuyen}</a></td>
                           </tr>`
                table.innerHTML += row
            }
        }
                ...
                if (name.indexOf(selected) > -1) {
                    ...
                    for (var j = 0; j < result.length; j++) {
                        var row = `<tr>
                            ...
                            <td><a href="https://${result[j].loptructuyen}" target="_blank">${result[j].loptructuyen}</a></td>
...
fetch("tkb.json")
    .then(response => response.json())
    .then(data => myArrayTKB = data)
    .then(() => {

        $('#search-tkb').on('keyup', function () {
            var value = $(this).val()
            var data = searchTable(value, myArrayTKB)
            buildTable(data)
        })
        ...
        function searchTable(value, data) {
            ...
            for (var i = 0; i < data.length; i++) {
                value = value.toLowerCase()
                v = nonAccentVietnamese(value)
                var name = data[i].tenhp.toLowerCase()
                n =nonAccentVietnamese(name)
                if (n.indexOf(v) > -1) {
        ...
        function buildTable(data) {
            console.log(data)
            var table = document.getElementById('tbTKB')
            table.innerHTML = ''
            for (var i = 0; i < data.length; i++) {
                var row = `<tr>
                            <td>${data[i].malhp}</td>
